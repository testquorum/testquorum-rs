...
"#;
        ...
        assert_eq!(
            ...
            Some(MavenConfig {
                enabled: true,
                pom_path: Some("backend/pom.xml".to_string()),
            })
        );
    }

    #[test]
    fn test_maven_default_when_omitted() {
        let toml = r#"
[managers]
autodetect = true
"#;
        let config = from_str(toml).unwrap();
        assert_eq!(config.managers.maven, None);
    }
